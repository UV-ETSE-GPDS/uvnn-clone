...
class NNBase(object):
    ...
    def compute_mean_loss(self, X, y):
        return self.compute_loss_full(X, y) / len(y)
        ...
        try:
            ...
            for idx, alpha in itertools.izip(idxiter, alphaiter):
                ...
                if counter % costevery == 0:
                    #self.grad_check(X[100], y[100])
                    #print self.params.W[0]
                    #import ipdb; ipdb.set_trace()
                    if devidx != None:
                        cost = self.compute_display_loss(X[devidx], y[devidx])
                    else: 
                        cost = self.compute_display_loss(X, y)
                    
                    if devX is not None:
                        costdev = self.compute_display_loss(devX, devy)
                    costs.append((counter, cost, costdev))
                    devmsg = ''
